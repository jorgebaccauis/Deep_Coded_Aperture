...
def Input_image_mat(image,CA,typ):
    im = loadmat(image).get('cube')
    im = measurements_cassi(np.expand_dims(im,0), CA, 100,typ)
...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, samples,PATH,batch_size=1,dim_input=(512, 512, 3), shuffle=True, dim_oput=(512, 512, 3), ty='mat',CA =0, typ='Binary_0'):
        'Initialization'
    ...
    def __data_generation(self, images_names):
        ...
        for i, file_name in enumerate(images_names):
            ...
            if(self.ty=='mat'):
                X[i,] = Input_image_mat(self.PATH + '/' + file_name,self.CA,self.typ)
...
def Build_data_set(IMG_WIDTH,IMG_HEIGHT,L_bands,L_imput,BATCH_SIZE,PATH,split_v,ty,CA,typ):
