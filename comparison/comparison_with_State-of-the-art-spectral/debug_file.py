...
from Read_Spectralv2 import *
import h5py

# %%

val_dir = '/home/hdsp/Documentos/Bases_datos/Small_proof/val/'
data_dir = '/home/hdsp/Documentos/Bases_datos/Small_proof/train/'
...
test_dataset = Build_data_set(M, M, L, L, batch_size, val_dir,memory_req=1.5)
...
coded_aperture = np.asanyarray((coded_aperture < 0.4) * 1, dtype=np.float32)
...
def HIDDSP(pretrained_weights=None, input_size=(512, 512, 31), depth=64, bands=31, H=0):
    inputs = Input(shape=input_size, name='X0')
...
model = HIDDSP(pretrained_weights=None, input_size=(512, 512, 31), depth=31, bands=31, H=coded_aperture)
...
check_path = './weights/First_model.h5'
...
history = model.fit(train_dataset, epochs=150, batch_size=2, validation_data=test_dataset, callbacks=[checkpoint])
