...
#-------------------------------------- DD- CASSI --------------------------------------------
class DD_CASSI_Layer(Layer):
    def __init__(self, output_dim=(256, 256, 12), input_dim=(256, 256, 12),type_code='Binary_0', trans=0.5, type_reg='Physical',parm1=0., batch_size=1, **kwargs):
        self.output_dim = output_dim
        self.input_dim = input_dim
        self.type_code = type_code
        self.type_reg = type_reg
        ...
        super(DD_CASSI_Layer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if self.type_code == 'Binary_0':
            #H_init = np.random.rand(1, self.kern, self.kern, self.shots)
            H_init = np.random.rand(1 , self.input_dim[0], self.input_dim[1] + self.input_dim[2] - 1, 1)

            H_init = tf.constant_initializer(H_init)
            self.H = self.add_weight(name='H', shape=(1,self.input_dim[0], self.input_dim[1] + self.input_dim[2] - 1, 1), initializer=H_init, trainable=True,
                                        regularizer=self.my_regularizer)
