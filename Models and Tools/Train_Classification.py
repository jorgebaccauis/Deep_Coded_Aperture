...
flags.DEFINE_string('architecture', 'Single_Pixel',
                    'name of the sensing architecture used [Single_Pixel,CASSI,CASSI_Color]')
...
flags.DEFINE_string('Net', 'LeNet5', 'Net structure for reconstruction, [Net, DeepCubeNet, UNetL-Net,LeNet5]')
flags.DEFINE_string('CA', 'Binary_1', 'Type of the Coded Aperture[Binary_0,Binary_1,Gray_scale]')  #

flags.DEFINE_string('type_reg', 'Correlation', 'Type of the Coded Aperture [Physical]')
flags.DEFINE_float('p', 1, 'family value')
flags.DEFINE_float('q', 1, 'family value')

flags.DEFINE_float('param_reg', 2e-5, 'Regularization parameter for first constraint (binary)')  # 1e-8
flags.DEFINE_float('param_reg2', 1e-6, 'Regularization parameter for the second regularizer')

flags.DEFINE_float('aument_reg', 15, 'Aumented parameter for regularzer param_reg *= aument_Red')  # 3e1
flags.DEFINE_integer('step_reg', 3, 'Regulator Increase Step')  # 1
...
def main(_argv):
    ...
    callbacks = [Aument_parameters2(p_aum=FLAGS.aument_reg, p_step=FLAGS.step_reg),
    ...
    if (FLAGS.Database == 'MNIST'):
        history = model.fit(x_train, y_train, epochs=FLAGS.epochs, batch_size=FLAGS.batch_size,
                            validation_data=(x_test, y_test), initial_epoch=last_epoch, callbacks=callbacks)
    ...
    if (FLAGS.Database == 'MNIST'):
        ...
        test_loss, Acc = model.evaluate(x_test, y_test, batch_size=64)
        print('Test loss: %.4f accuracy: %.4f' % (test_loss, Acc))
