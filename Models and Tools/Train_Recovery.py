...
flags.DEFINE_string('Net','UNetL','Net structure for reconstruction, [Net, DeepCubeNet, UNetL-Net]')
...
flags.DEFINE_float('param_reg',1e-9, 'Regularization parameter for first constraint (binary)')
...
def main(_argv):
    ...
    #callbacks = [Aument_parameters(p_aum=FLAGS.aument_reg, p_step=FLAGS.step_reg),SaveLastEpochWeights(FLAGS.results), tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)]
    ...
    model.compile(optimizer=optimizad, loss='mean_squared_error',metrics=[psnr,'mse','mae',SSIM])
    ...
    if(FLAGS.Database=='MNIST'):
        ...
        for i in range(0,x_test.shape[0],100):
            ...
            psnr_v.append(psnr_a.numpy()); mse_v.append(mse_a.numpy());  mae_v.append(mae_a.numpy());  ssim_v.append(ssim_a.numpy())
    ...
    if (FLAGS.Database == 'Data_store'):
        ...
        for inp,tar in test_dataset.take(-1):
            ...
            psnr_v.append(psnr_a.numpy()); mse_v.append(mse_a.numpy());
            mae_v.append(mae_a.numpy()); ssim_v.append(ssim_a.numpy())
